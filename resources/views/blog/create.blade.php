...
@section('title', 'Managee - Tulis Blog Baru')
...
<div class="write-blog-page">
    <div class="write-container">
        ...
        <div class="mock-header">
            <div>
                <h1 class="mock-header-title">Tulis Blog Baru</h1>
            </div>
        ...
        <form action="{{ route('blog.store_public') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mock-form-group">
                ...
                <input type="hidden" name="category" id="selected-category" value="{{ old('category') }}" required oninvalid="this.setCustomValidity('Silakan pilih kategori.')" oninput="this.setCustomValidity('')">
                
                <div class="category-chips-grid">
                    <button type="button" class="category-form-chip {{ old('category') === 'Panduan Pemilik' ? 'active' : '' }}" data-value="Panduan Pemilik">
                    ...
                    <button type="button" class="category-form-chip {{ old('category') === 'Panduan Penyewa' ? 'active' : '' }}" data-value="Panduan Penyewa">
                    ...
                    <button type="button" class="category-form-chip {{ old('category') === 'Hukum & Regulasi' ? 'active' : '' }}" data-value="Hukum & Regulasi">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg>
                        <span>Hukum & Regulasi</span>
                    </button>
                    <button type="button" class="category-form-chip {{ old('category') === 'Dekorasi & Renovasi' ? 'active' : '' }}" data-value="Dekorasi & Renovasi">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
                        <span>Dekorasi & Renovasi</span>
                    </button>
                    <button type="button" class="category-form-chip {{ old('category') === 'Kuliner' ? 'active' : '' }}" data-value="Kuliner">
                    ...
                    <button type="button" class="category-form-chip {{ old('category') === 'Perawatan & Fasilitas' ? 'active' : '' }}" data-value="Perawatan & Fasilitas">
                    ...
                    <button type="button" class="category-form-chip {{ old('category') === 'Lainnya' ? 'active' : '' }}" data-value="Lainnya">
            ...
            <div class="mock-form-group">
                ...
                <input type="text" name="title" id="title" class="mock-input" placeholder="Tulis judul yang menarik perhatian..." value="{{ old('title') }}" required oninvalid="this.setCustomValidity('Harap isi kolom ini.')" oninput="this.setCustomValidity('')">
                @error('title')
                    <span class="form-error-msg">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mock-form-group">
                ...
                <textarea name="summary" id="summary" class="mock-input mock-textarea" placeholder="Deskripsikan isi artikel dalam 1-2 kalimat...">{{ old('summary') }}</textarea>
                @error('summary')
                    <span class="form-error-msg">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mock-form-group">
                ...
                <textarea name="content" id="content" class="mock-input mock-textarea mock-textarea-content" placeholder="Tulis isi artikel Anda di sini..." required oninvalid="this.setCustomValidity('Harap isi kolom ini.')" oninput="this.setCustomValidity('')">{{ old('content') }}</textarea>
                @error('content')
                    <span class="form-error-msg">{{ $message }}</span>
                @enderror
            </div>
            ...
            <!-- Tags Input -->
            <div class="mock-form-group">
                <label for="tags" class="mock-label">Tags (pakailah #)</label>
                <input type="text" name="tags" id="tags" class="mock-input" placeholder="Contoh: #Apartemen #Kontrak #Panduan" value="{{ old('tags') }}">
                @error('tags')
                    <span class="form-error-msg">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mock-form-group">
                <label class="mock-label">Gambar Sampul (Opsional)</label>
                <div style="display: flex; gap: 1rem; align-items: stretch; margin-bottom: 0.5rem; flex-wrap: wrap;">
                    <!-- File Upload Button -->
                    <label for="image-file-input" class="upload-box" style="flex: 1; min-width: 200px; display: flex; align-items: center; justify-content: center; margin-bottom: 0; min-height: 52px;">
                        ...
                        <span id="file-name-display" style="text-overflow: ellipsis; overflow: hidden; white-space: nowrap; max-width: 180px;">Pilih File Komputer</span>
                        <input type="file" name="image_file" id="image-file-input" accept="image/*" style="display: none;" onchange="document.getElementById('file-name-display').innerText = this.files[0] ? this.files[0].name : 'Pilih File Komputer'">
                    </label>
                    
                    <div style="display: flex; align-items: center; color: var(--text-light-slate); font-size: 0.85rem; font-weight: bold; justify-content: center;">atau</div>
                    
                    <!-- URL Input Box -->
                    <div class="upload-box" style="flex: 1.5; min-width: 250px; display: flex; align-items: center; margin-bottom: 0; min-height: 52px;" onclick="document.getElementById('image-url-input').focus()">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right: 0.5rem; flex-shrink: 0;">
                            <circle cx="12" cy="12" r="10"></circle>
                            <line x1="2" y1="12" x2="22" y2="12"></line>
                            <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path>
                        </svg>
                        <input type="url" name="image" id="image-url-input" class="mock-input" style="padding: 0; background: transparent; border-radius: 0; font-size: 0.95rem; color: var(--text-dark);" placeholder="Masukkan URL Gambar..." value="{{ old('image') }}">
                    </div>
                </div>
                <span style="font-size: 0.75rem; color: var(--text-light-slate); margin-top: 0.15rem; margin-left: 0.5rem;">
                    Unggah file gambar dari komputer atau masukkan tautan URL gambar. Jika dikosongkan, gambar default otomatis digunakan.
                </span>
                @error('image')
                    <span class="form-error-msg">{{ $message }}</span>
                @enderror
            ...
            <div class="mock-footer">
                ...
                <button type="submit" class="btn-mock-submit">Publikasikan</button>
            </div>
    ...
    if (tagsInput) {
        function formatTags() {
            ...
            let formatted = words.map(word => {
                word = word.trim();
                if (word === '') return '';
                // If the word doesn't start with '#', prepend it
                if (!word.startsWith('#')) {
                    return '#' + word;
                }
                return word;
